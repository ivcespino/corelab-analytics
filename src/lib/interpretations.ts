...
export function interpretPearson(r: PearsonResult, x: string, y: string): Interpretation {
  ...
  const ranSentence = `Pearson correlation between **${x}** and **${y}** with n = ${r.n}, ${tailLabel[r.tail]} test at ${pct(r.confidence)} confidence.`;
  ...
  const chart = `The scatter plot shows each observation as a point with **${x}** on the X axis and **${y}** on the Y axis. The straight line is a least-squares trend line — it ${r.r >= 0 ? "slopes upward" : "slopes downward"}, meaning the two variables tend to ${slopeWord}. Points ${tightness}, which reflects how reliable the relationship is.`;
  ...
  const implies = sig
    ? `At ${pct(r.confidence)} confidence, there is evidence of a linear association between ${x} and ${y}: as ${x} ${r.r >= 0 ? "increases" : "increases"}, ${y} tends to ${r.r >= 0 ? "increase" : "decrease"}. Note that correlation does not establish causation — a third variable could drive both.`
    : `At ${pct(r.confidence)} confidence, there is **insufficient evidence** to conclude a linear association between ${x} and ${y}. This does not prove they are unrelated; the sample may simply be too small or the relationship non-linear.`;
...
export function interpretRegression(r: RegressionResult): Interpretation {
  const alpha = 1 - r.confidence;
  const ran = `${r.k === 1 ? "Simple" : "Multiple"} linear regression predicting **${r.response}** from ${r.predictors.map((p) => `**${p}**`).join(", ")} with n = ${r.n}, ${r.hasIntercept ? "intercept included" : "no intercept"}, at ${pct(r.confidence)} confidence.`;
  const chart = `The plot shows **observed vs. predicted ${r.response}**. Each point is one row of your data: the X coordinate is what the model predicted, the Y coordinate is the actual value. The dashed diagonal is the line of perfect prediction — points hugging this line mean the model fits well; points scattered far from it mean large residual errors.`;
  ...
  const numbers = `R² = ${fmt(r.rSquared, 3)} means the model explains **${(r.rSquared * 100).toFixed(1)}%** of the variance in ${r.response} (Adjusted R² = ${fmt(r.adjustedRSquared, 3)}). The overall F(${r.k}, ${dfRes}) = ${fmt(r.fStatistic, 2)}, p = ${fmtP(r.fPValue)} is **${fSig ? "statistically significant" : "not statistically significant"}** at α = ${fmt(alpha, 2)}. ${sigCoefs.length} of ${r.predictors.length} predictor(s) are individually significant.`;
  const coefSentences = r.coefficients
    ...
    .map((c) => {
      ...
      return sig
        ? `A 1-unit increase in **${c.name}** is associated with a ${fmt(Math.abs(c.b), 3)}-unit ${dir} in ${r.response} (B = ${fmt(c.b, 3)}, ${pct(r.confidence)} CI [${fmt(c.ciLower, 3)}, ${fmt(c.ciUpper, 3)}], p = ${fmtP(c.pValue)}).`
        : `**${c.name}** is not a significant predictor (B = ${fmt(c.b, 3)}, CI crosses 0: [${fmt(c.ciLower, 3)}, ${fmt(c.ciUpper, 3)}], p = ${fmtP(c.pValue)}).`;
    })
    .join(" ");
...
export function interpretCronbach(r: CronbachResult, items: string[]): Interpretation {
  const ran = `Cronbach's α reliability analysis on ${r.k} items (${items.join(", ")}) across ${r.n} respondents at ${pct(r.confidence)} confidence.`;
  ...
  const chart = `The bar chart plots the **mean response for each item**. Bars of similar height suggest items behave consistently; a bar that stands far above or below the others may be tapping a different construct.${outliers.length ? ` Items to inspect: **${outliers.join(", ")}**.` : ""}`;
  const verdict =
